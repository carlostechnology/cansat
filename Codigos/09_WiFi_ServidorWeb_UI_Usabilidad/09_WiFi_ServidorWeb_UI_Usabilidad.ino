/***************************************************
  Proyecto: CANSAT - Servidor Web (UI mejorada + usabilidad)
  Versión: v1.0 - 27/10/2025
  Autor: Equipo CANSAT
  Centro: IES Gonzalo Torrente Ballester (Salamanca)

  Descripción:
  Servidor web en ESP32 con HTML/CSS/JS embebido y mejoras
  de usabilidad: botones sin recarga, consulta de estado /state
  en JSON y sincronización periódica entre clientes.
...
<style>
  :root{
    --bg1:#0f172a; --bg2:#111827; --txt:#e5e7eb; --muted:#9ca3af;
    --ok:#22c55e; --off:#ef4444; --accent:#38bdf8;
  }
  *{box-sizing:border-box}
  ...
  /* Hover solo en dispositivos con puntero (evita doble toque en iOS) */
...
<body>
  <main class="card">
    ...
    <div class="row">
      <span class="gpio">Pin: <b>%GPIO%</b></span>
      <span class="pill %CLASS%">LED: <span id="stateTxt">%STATE%</span></span>
    </div>
  ...
  <script>
    ...
    const stateSpan = document.getElementById('stateTxt');
    const btnOn  = document.getElementById('btnOn');
    const btnOff = document.getElementById('btnOff');

    function render(on){
      pill.className = 'pill ' + (on ? 'ok' : 'off');
      stateSpan.textContent = on ? 'ENCENDIDO' : 'APAGADO';
    }
    ...
    async function setLed(on){
      // deshabilita momentáneamente para evitar toques repetidos
      btnOn.disabled = btnOff.disabled = true;
      try{
        // Permitimos GET o POST; aquí POST semántico
        await fetch(on ? '/on' : '/off', { method:'POST', cache:'no-store' });
        await getState(); // confirmamos estado real del ESP32
      }catch(e){ console.error(e); }
      btnOn.disabled = btnOff.disabled = false;
    }
    btnOn.addEventListener('click', ()=>setLed(true));
    btnOff.addEventListener('click',()=>setLed(false));

    // sincroniza todos los clientes cada 1 s
...
void handleState() {
  sendNoStore();
  bool on = digitalRead(LED_PIN);
  String j = String("{\"on\":") + (on ? "true" : "false") + "}";
  server.send(200, "application/json", j);
}

void handleOn() {
  digitalWrite(LED_PIN, HIGH);
  handleState(); // respondemos con el estado actual en JSON
}
...
void setup() {
  ...
  // Acepta GET o POST para máxima compatibilidad de navegadores
  server.on("/on",  HTTP_ANY, handleOn);
  server.on("/off", HTTP_ANY, handleOff);
